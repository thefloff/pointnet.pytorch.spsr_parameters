...
class ABCDataset(data.Dataset):
    def __init__(self,
        ...
        self.split = split
        self.fns = []
        self.test_every = 1 / test_fraction
    ...
    def __getitem__(self, index):
        ...
        point_set = np.load(cloud[0]).astype(np.float32)
        ...
        point_set = torch.from_numpy(point_set)
        gt_vector = torch.from_numpy(np.array(gt_vector).astype(np.float32))
        return point_set, gt_vector, cloud[2]
